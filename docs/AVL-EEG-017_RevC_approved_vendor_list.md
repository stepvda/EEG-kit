...
| L1 | 1 | Ferrite bead 600 Ω at 100 MHz, 0603 | Murata **BLM18AG601SN1D** | 81-BLM18AG601SN1D | PREFERRED | TDK MPZ1608S601A | 600 Ω ±25 % at 100 MHz, **500 mA**, DC resistance 0.38 Ω. **Corrected under ECO-EEG-031.** Rev B named `BLM18PG601SN1D` at 1.5 A, and there is no such part: `PG` is Murata's large-current power-supply series, which is not made at 600 Ω in 1608 -- its 0603 range stops at 470 Ω (1 A). The impedance code itself was right; a reviewer read `601` as some other impedance, and it does mean 600 Ω. The 1.5 A came with the wrong part number and has no requirement behind it: L1 feeds VDD_ISO, the ADuM4160 module's device-side supply, which is tens of milliamperes, so 0.38 Ω costs about 10 mV. |
...
| U1, U2, U3 | 3 | Quad precision CMOS op-amp, rail-to-rail I/O, 5.5 V absolute maximum -- rectifier, absolute-value summer, 50 Hz filter, output buffer | Texas Instruments **OPA4376AIPWR** | 595-OPA4376AIPWR | **TSSOP-14 (PW)**, 2500/reel | 1 | PREFERRED | none | **No alternate, and corrected under ECO-EEG-031.** The quad OPA4376 is offered in **one** package, TSSOP-14 (PW). `OPA4376AIDR` is not an orderable part number -- the `D` suffix is SOIC and there is no SOIC-14 quad -- so Rev B's line could not have been bought. Rev A of this file quoted OPA4376AIPWR, which is the correct part, and withdrew it on the ground that it did not fit the SOIC-14 land pattern `design.py` then placed; **that withdrawal is itself withdrawn**, and `design.py` now places `TSSOP-14_4.4x5mm_P0.65mm`. Pin numbering is identical between the two packages, so no net changed: the netlist is byte-identical to Rev B's. The part runs from the ADS1299 module's ±2.5 V rails -- 5.0 V total -- and must be rail-to-rail on both input and output with an absolute maximum at or above 5.5 V. Rev A's dual OPA2376 is superseded by ECO-EEG-004 and must not be re-introduced. Quiescent current 760 µA per amplifier, 3.04 mA per package *(calculated)*, inside the ICD-EEG-006 §2.1 rail budget. |
...
**J15–J17 are patient-connected parts under IEC 60601-1 and are not substitutable.** Added
under ECO-EEG-031: `design.py` now carries that statement in `NOT_SUBSTITUTABLE`, so the
generated BOM prints "NOT SUBSTITUTABLE -- E-09, S-02: PATIENT-CONNECTED under IEC 60601-1. A
touch-proof DIN 42802 socket approved in writing from a sample. No 'or equivalent'" on the
line itself. A buyer reading the BOM alone can no longer treat the line as an ordinary
connector. `tools/footprint_audit.py` reports these three as **OPEN** rather than as passes,
for the reason above: there is no drawing to check the footprint against until a sample is
approved.
...
| OPA4376AIPWR | tube or cut tape at Phase 1 | Full reel 2500 | 7 / 41 / 101 / 199 | Cut tape at Phase 1, one reel at Phase 2. Single-source part (§3). |

...
**Document:** DSN-EEG-003 **Revision:** D **Date:** 1 September 2026
...
**Supersedes:** Rev C of this document in its entirety, which superseded Rev B
...
| Validated production-test specification | TST-EEG-004 Rev C, standalone, with fixtures in JIG-EEG-009 Rev C |
...
## 3. Carrier board EEG-CAR-01

### 3.1 What is in the package

**Two revisions are in the tree and only one of them is current.** Rev B is the routed
board, withdrawn from fabrication on 4 September 2026 (ECO-EEG-030) and kept as history.
Rev C is the current carrier: the same circuit at corrected footprints and part numbers,
**unrouted**, released to an external layout contractor. `tools/emit_all.py` writes the Rev C
set and **refuses** to rewrite the Rev B set, because `design.py` has moved to Rev C pads and
regenerating Rev B from it would produce a set whose copper does not reach its own pads.

**Rev C -- the current set, all generated by `tools/emit_all.py`**

| File | Content |
|---|---|
| `kicad/EEG-CAR-01_RevC.kicad_pro` | KiCad project: eight net classes, an assignment for all 156 nets, board minima, DRC severities |
| `kicad/EEG-CAR-01_RevC.kicad_sch` and nine sheets | the **native hierarchical schematic**, KiCad 8. All 211 designators |
| `kicad/EEG-CAR-01.kicad_sym` | the symbol library those sheets are drawn from |
| `kicad/EEG-CAR-01_RevC.kicad_pcb` | the board: placed, netted, 37 footprints locked, **no copper** |
| `kicad/EEG-CAR-01_RevC.kicad_dru` | 37 custom design rules, from `tools/rules.py` |
| `kicad/EEG-CAR-01_RevC-IPC-D-356A.ipc` | the netlist, 156 nets, from the pads alone |
| `kicad/EEG-CAR-01_RevC_BOM.csv` | grouped BOM with a Substitution column |
| `kicad/EEG-CAR-01_RevC_CPL_SMT_top.csv`, `..._CPL_THT_top.csv` | **PROVISIONAL** placement; the files say so on their first line |
| `kicad/EEG-CAR-01_RevC_outline_and_fixed_connectors.dxf` | outline, mounting holes and keep-outs, the isolation strip, thirty connector courtyards |
| `kicad/EEG-CAR-01_RevC_schematic_netlist_check.txt` | the schematic read back and diffed against `design.py` |
| `mech/step/` | 3D bodies: 22 footprint classes bound into the board, the carrier envelopes, MP-01 and its standoffs, the module envelopes |
| `mech/EEG-CAR-01_RevC_collision_check.txt` | the interference report |
| `docs/footprint_audit_RevC.md` | all 211 designators against their part numbers |
| `docs/LAY-EEG-034` | the layout rule sheet, part of the order specification |
| `kicad/RevC_layout_inputs/` | the 51-file handover set with a SHA-256 for every file |

**Rev B -- history. Not a fabrication set, not a quotation basis, not regenerated.**

| File | Content |
|---|---|
| `kicad/EEG-CAR-01_RevB_routed.kicad_pcb` | routed board: tracks on L1 and L4, through vias, reference planes on L2 and L3 |
| `kicad/gerber/*.gbr`, `*.drl`, `*.ipc` | Gerber X2, Excellon 2, IPC-D-356A |
| `kicad/EEG-CAR-01_RevB_DRC_report.txt` | every rule the Rev B rule set contained, measured. **See 3.4 and read it with `..._regraded_ECO-EEG-032.txt`** |
| `kicad/EEG-CAR-01_RevB_regraded_ECO-EEG-032.txt` | the same geometry under the rule set the review asked for: **1 311 occurrences** |
| `kicad/EEG-CAR-01_RevB_BOM.csv`, the two CPLs, the three drawings | as released on 2 September 2026 |
| `schematic/SCH-EEG-005_RevB_schematic_set.pdf` | eight sheets. A picture, not a netlist -- finding 7 |
| `tools/` | the generator, the router, the DRC and this board's source |
...
### 3.4 How the routing was produced, what that means, and what happens to Rev C

**Rev C has no routing and this section has no result to transcribe for it.** What follows is
the Rev B result, kept because it is the measured state of the data in the tree, and then what
Rev C will be graded on when it comes back.
...
**And the rule set was incomplete, which is the part that matters.** The report above says
"VIOLATIONS: 0 -- none. The board passes every rule listed above", and it is correct: it is
correct about the twelve rules it had. Six of the seven findings the external reviewer made
are rules it did not have, and there was no way for a report to say so. **ECO-EEG-032 encoded
them and regraded the same released geometry**; the result is
...
The Rev B report is not withdrawn. It measured what it was asked to measure.

**What Rev C is graded on when it comes back.** The rule set is `tools/rules.py`, the sheet the
contractor works under is **LAY-EEG-034**, and the machine-readable half is
`kicad/EEG-CAR-01_RevC.kicad_dru`. On return this programme runs `tools/drc.py` with
`tools/drc_geometry.py` over the returned geometry -- the twelve original rules **and** the six
above, including the two KiCad has no constraint for and cannot check: the 90-degree angle
floor and pad-centre entry. The three conditions of the fabrication-release gate in ECO-EEG-016
section 3 are then measured on that geometry, and the placement review of RFQ-EEG-002A must
already have closed, before routing was confirmed.
...
provisioning script and the host verification tool, not only `main.c`. FW-EEG-001 Rev D is
...
Now a standalone controlled specification with fixtures designed in JIG-EEG-009 Rev C.
...
* **The fixtures are FIX-01 to FIX-04**, with sub-assemblies lettered FIX-01/A to FIX-01/G,
  ...
  TST-EEG-004 Rev C section 6.1**, which is its only home; JIG-EEG-009 Rev C designs the
...
| Can you provision and test a unit? | **Yes**, once the firmware image exists | TST-EEG-004 Rev C and JIG-EEG-009 Rev C are complete; the image is programme work |
...
> DSN-EEG-003 Rev D section 9. We would rather tell you that than have you find them.
>

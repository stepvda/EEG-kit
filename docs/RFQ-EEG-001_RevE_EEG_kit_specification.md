...
**Governing document:** DSN-EEG-003 Rev D. Where this document and DSN-EEG-003 differ,
...
an open item. The rulings themselves are the controlled document **RUL-EEG-021 Rev B**.
...
unbuildable. They are listed in DSN-EEG-003 Rev D section 9 and each is logged in ECO-EEG-016.
...
DSN-EEG-003 Rev D section 3.3 asks for and which a swiss-cheesed two-layer pour cannot deliver. The
...
- **RFQ-EEG-002A -- layout review.** Review the supplied routing of EEG-CAR-01 Rev B against
  the rules in DSN-EEG-003 Rev D section 3.3, correct what is wrong, and sign it off. Deliver
...
track and clearance minima, hole schedule, zone split and IPC classes -- is in **DSN-EEG-003 Rev D
section 3.2**, and the isolation keep-out and star-point rules are in **DSN-EEG-003 Rev D section
...
| E-08 | Analogue and digital ground partition, guard pour around all high-impedance inputs, no digital trace inside the analogue zone. The four-layer stack gives a continuous AGND_REF reference under every analogue trace on both inner layers. Met by the zoning rules of DSN-EEG-003 Rev D section 3.3 and verified in the DRC report | M |
...
| E-24 | ADuM4160-based USB isolator module (≥ 2.5 kV RMS) at J10, with the host connector on the module and no carrier copper crossing the barrier; the isolation keep-out is specified in DSN-EEG-003 Rev D section 3.3. The part is not substitutable. **This requirement asks for USB-C and the only named candidate module presents USB-B. That is a live non-conformance.** The interim answer is a short USB-B-to-USB-C panel pigtail, **WH-09**, until an isolator module with a USB-C host connector is qualified; it is not a settled design | M |
| E-25 | Analogue supplies **AVDD = +2.5 V and AVSS = −2.5 V**, generated on ADS1299 module #1 and brought onto the carrier at J23. AGND_REF is the analogue 0 V mid-rail, not a ground; the single star point to DGND is the rule in DSN-EEG-003 Rev D section 3.3. A buck-boost converter supplies the 5 V rail; it is in the digital zone, at least 15 mm from any input net. *Its switching frequency is not measured in production* | M |
...
- **Modules on jumpers rather than plugged directly** (section 2). The cost is stated openly in
  DSN-EEG-003 Rev D section 2.1. Jumper keying is decided: a shrouded polarised header at the

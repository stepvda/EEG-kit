...
This directory is the **KiCad 10** emission: `(kicad_sch (version 20260306))` and
`(kicad_pcb (version 20260206))`, `(generator_version "10.0")`.

**This is the set for the JLCPCB layout desk, which has confirmed it runs KiCad 10. The KiCad 8 set is kept alongside at `kicad/RevC_layout_inputs/` for anyone still on 8.**
...
**This set was opened by KiCad 10.0.6 and DRC was run against the `.kicad_dru`.** It loads with no schema warning and no rule-parse error. The report is in this directory as `drc_kicad10.txt` and is read in check 9 of `EQUIVALENCE_kicad8_vs_kicad10.md` -- including what the counts in it do and do not mean.

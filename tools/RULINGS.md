# Package v2 — rulings on every open question and disagreement

**These rulings are binding.** They were made after the cross-document audit of
2026-09-01 found that seventeen documents, written from one fact base, still disagreed with
each other in sixty places. Where a document disagrees with a ruling below, the document is
wrong and is corrected. Where a ruling disagrees with `tools/design.py` or `tools/mech_gen.py`,
the source file is right and the ruling here is a transcription error — say so.

Every ruling is written as: **the answer**, then why, then what to change.

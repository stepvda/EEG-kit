...
NOTES = {
    "fabrication": [
        ...
        "7a. EVERY HOLE SIZE IN THIS PACKAGE IS A FINISHED DIAMETER, AFTER PLATING.  The",
        "    fabricator selects the drill and MUST DECLARE THE TOOL SIZE AND THE PLATING",
        "    ALLOWANCE IT USED, per lot, with the lot documents of note 14.  This is not a",
        "    formality: the INTERNAL annular ring of note 7b is measured to the DRILLED hole",
        "    wall, not to the finished one, so the tool size is what decides whether the",
        "    board meets it and the package cannot compute the figure without it.",
        "7b. ANNULAR RING.  The EXTERNAL ring on a 0.60 mm via pad with a 0.30 mm finished",
        "    hole is 0.15 mm.  The INTERNAL ring is (0.60 - drilled) / 2 and is SMALLER:",
        "    at a 0.05 mm total allowance it is 0.125 mm, at 0.10 mm it is 0.100 mm.  The",
        "    0.15 mm figure quoted elsewhere in this package is the EXTERNAL ring; where a",
        "    document uses it as the internal ring it is optimistic.  QP-EEG-010 IQC-B11",
        "    accepts an internal ring of 0.025 mm minimum, so the misregistration budget is",
        "    (internal ring - 0.025) mm and it falls as the tool grows.",
